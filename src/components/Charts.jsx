  ...
  return (
    <div className="donut-wrap">
      <svg width={size} height={size} viewBox="0 0 148 148">
        <circle cx="74" cy="74" r={r} fill="none" stroke="#eef0f3" strokeWidth="16" />
        {slices.map((sl) => {
        ...
        {center && (
          <text x="74" y="70" textAnchor="middle" fontSize="11" fill="#6d7178">
            {center.top}
          </text>
        )}
        {center && (
          <text x="74" y="90" textAnchor="middle" fontSize="15" fontWeight="650" fill="#111214">
            {center.bottom}
          </text>
        )}
      </svg>
    ...
    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 10, height: 140, padding: '8px 4px 0' }}>
      {series.map((s) => (
        <div key={s.label} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
          ...
          <div style={{ fontSize: 11, color: '#6d7178' }}>{s.label}</div>
        </div>
      ))}
    </div>
  )
